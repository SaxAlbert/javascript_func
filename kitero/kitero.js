...
function hideBasedOnSelected(dropdownList){
    const a=dropdownList.parentElement
    const b=a.querySelectorAll(".card")
    for(const e of b){
        e.classList.add('hide')
    }
    makeVisibleBasedOnId(a,dropdownList.value)
}
...
function hideBasedOnCheckbox(checkbox){
    ...
    const b=checkbox.parentElement
    if(a){
        makeInvisibleBasedOnId(b,'no')
        makeVisibleBasedOnId(b,'ferfi')
    }
    else{
        makeInvisibleBasedOnId(b,'ferfi')
        makeVisibleBasedOnId(b,'no')
    }
}
